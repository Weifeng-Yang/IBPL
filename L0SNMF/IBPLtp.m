...
function [var,loss,timerun,ats,bts]=IBPLtp(var,ngmar,aa,maxiteropt,at,bt,obj,stopindex,r,tk,btmax,rho)
...
varK=var;
% for j=1:length(var)
%     vartance=vartance+norm(var{j}-varK{j});
% end
...
check=abs(sqrt(loss(i+1)*2)-sqrt(loss(i)*2))/returnloss;
if(check<1e-3)
    btmax=btmaxtemp;
end
